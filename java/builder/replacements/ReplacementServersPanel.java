...
public class ReplacementServersPanel extends JScrollPane
{
	...
	// Working things
	private java.util.List<ConfigServerType> mServers;
	private int mNetDistance;
	private ConfigServerType mWorkingServer;
	...
	private JSpinner mServerAvailableFrom;
	...
	public ReplacementServersPanel(BuilderPreferences pPrefs, JFrame parent, java.util.List<ConfigServerType> pServ)
	{
		mPrefs = pPrefs;
		parentPanel = parent;
		mServers = pServ;
		...
		topPanel.setPreferredSize(new Dimension(MainWindow.WINDOW_WIDTH-400, MainWindow.WINDOW_HEIGHT-150)); //TODO AS 30.7.12- replace gridlayout with gridbag to fix this 'hack'
		topPanel.add(mMainPanel);
	...
	private void setupMainPanel()
	{
		...
		svControlPanel.setLayout(new BoxLayout(svControlPanel,
				BoxLayout.LINE_AXIS));
		mMainServerAddBtn = new JButton("Add", mPrefs.getImage("server_add.png"));
		mMainServerAddBtn.addActionListener(event);
		svControlPanel.add(mMainServerAddBtn);
		svControlPanel.add(Box.createHorizontalGlue());
		mMainServerDelBtn = new JButton("Remove", mPrefs.getImage("server_delete.png"));
	...
	private void setupServerPanel()
	{
		...
		mServerCPUCpyBtn = new JButton("Copy", mPrefs.getImage("cpu_copy.png"));
		...
		mServerPanel.add(Box.createVerticalStrut(10));
		mServerPanel.add(getPanelOfText(new JLabel("Time Avalable From")));
		mServerPanel.add(Box.createVerticalStrut(10));
		mServerAvailableFrom = new JSpinner(new SpinnerNumberModel(0, 0,
				Double.MAX_VALUE, 1));
		mServerAvailableFrom
				.setMaximumSize(new Dimension(Integer.MAX_VALUE, 15));
		mServerAvailableFrom.addChangeListener(event);
		mServerPanel.add(mServerAvailableFrom);
	...
	private int getID(ConfigServerType s)
	{
		for (int i = 0; i < mServers.size(); i++)
		{
	...
	private void populateMain()
	{
		...
		ConfigServerType current = mWorkingServer;
	...
	private void populateServer()
	{
		if (mWorkingServer != null)
		{
			...
			mServerAvailableFrom
					.setValue(mWorkingServer.getTimeAvailableFrom());
			mServerAvailableFrom.setEnabled(true);
		...
		} else
		{
			...
			mServerAvailableFrom.setValue(0);
			mServerAvailableFrom.setEnabled(false);
			// RAM
			...
			ChangeListener, DocumentListener
	{
		...
		public void actionPerformed(ActionEvent e)
		{
			...
			{
				mServers.add(new ConfigServerType());
				populateMain();

			} else if (source.equals(mMainServerDelBtn))
			{
				int result = JOptionPane.showConfirmDialog(parentPanel,
		...
		private void updated()
		{
			if (mWorkingServer != null && !mServerModifiying)
			{
				...
				mWorkingServer.setSize(((Number) mServerSize.getValue())
						.intValue());
				mWorkingServer.setMeanFailTime(((Number) mServerMeanFailureTime
						.getValue()).longValue());
				...
				ram.setSpeed(((Number) mServerRAMSpeed.getValue())
						.doubleValue());
				mWorkingServer
						.setTimeAvailableFrom(((Number) mServerAvailableFrom
								.getValue()).intValue());
	...
	private class ServerListListener implements ListSelectionListener
	{
		...
		public void valueChanged(ListSelectionEvent e)
		{
			...
			{
				mWorkingServer = mServers
						.get(mMainServerList.getSelectedIndex());
				mWorkingCPU = null;
			} else
			{
				mWorkingServer = null;
