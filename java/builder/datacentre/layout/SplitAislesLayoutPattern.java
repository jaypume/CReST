...
public class SplitAislesLayoutPattern extends AbstractLayoutPattern
{
	// Constants
	private static final int SPLIT_AISLE_LENGTH = 10;
	private static final boolean ALTERNATE_AISLE_DIRECTION = true;
	
	public SplitAislesLayoutPattern()
	{
		super(LayoutPatternType.SPLIT_AISLES);
	}
	...
	public void generateLayout(ConfigDatacentre pGeneratedDatacentre, int[] pNetworkDistances, List<DatacentreGeneratorProgressListener> listeners)
	{
		...
		for (int i = 0; i < mAisles; i++)
		{
			...
			// If using aircon, and for every other aisle
			if (mTopAirCon && (i % 2 == 0))
			{
				ConfigAirCon air0 = new ConfigAirCon();
				air0.setLocation(new Point(0, 1)); // offset y value so aircon blows down the aisle walkway
			...
			{
				// If using split aisles, and it's time to leave a gap, don't place anything here
				if ((j % (SPLIT_AISLE_LENGTH + 1) == 0))
				{
					continue;
				}

				// Otherwise pick a random rack
