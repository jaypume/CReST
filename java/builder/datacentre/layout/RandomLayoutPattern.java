...
public class RandomLayoutPattern extends AbstractLayoutPattern
{
	// Constants
	private static final double SPACE_PERCENTAGE = 1.0 / 3;
	private static final boolean ALTERNATE_AISLE_DIRECTION = true;

	public RandomLayoutPattern()
	{
		super(LayoutPatternType.RANDOM);
	}
	...
	public void generateLayout(ConfigDatacentre pGeneratedDatacentre, int[] pNetworkDistances, List<DatacentreGeneratorProgressListener> listeners)
	{
		...
		for (int i = 0; i < mAisles; i++)
		{
			...
			{
				//Calculate probability of placing a rack here
				int x = (int) (1 / SPACE_PERCENTAGE);
				if(r.nextInt(x) % x != 0)
				{				
					//Pick a random rack
