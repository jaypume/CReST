...
/**
 * Class for reading in simulation parameters for the market configuration from a configuration .properties file...
 */
public class MarketConfigParams {
	 ...
	 * @return true
	 */
	public static MarketSettings update(MarketSettings settings) {
				
		logger.info("Updating market settings using config parameters: " + prop);
	...
	/**
	 * Copy config parameters file to the log directory
	 * @param logManager - the log manager for the simulation run
	 * @return true if file copied, false if error
	 */
	public static boolean copyConfigFileToResultsDirectory(LogManager logManager) {
		
    	logger.debug("Copying params file to log directory...");
    	try {
	    	String dest = logManager.getLogResultsDirName()+"/"+new File(filename).getName();
	    	File fOrig = new File(filename); //Strip directory path from filename
	    	File fDest = new File(dest);
	    	FileUtils.copyFile(fOrig,fDest);
	    	logger.info("Params file copied to: '" + fDest.getAbsolutePath() + "'");
	    	System.out.println("Results directory: " + logManager.getLogResultsDirName());
	    	return true;
